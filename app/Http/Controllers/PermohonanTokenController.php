...
class PermohonanTokenController extends Controller
{
    ...
    public function cetakPermohonan($permohonan_id)
    {
        ...
        $datafix['rt'] = $data->penduduk->rt->nomor ?? '-';
        $datafix['rw'] = $data->penduduk->rt->rw->nomor ?? '-';

        $kepalaDesa = Penandatangan::where('kepala_desa', 1)->first();
        $datafix['nama_kepala_desa'] = $kepalaDesa->nama ?? '-';
        $datafix['nip_kepala_desa'] = $kepalaDesa->nip ?? '-';
